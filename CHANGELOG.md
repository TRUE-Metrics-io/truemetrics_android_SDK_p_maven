...
- A sensor can now be given its own upload period in the configuration, shorter than the regular upload period. Its readings are then uploaded on that cadence, separately from the regular batch and ahead of any backlog. Readings the SDK could not send within a minute (or within the regular upload period, if that is longer), for example while offline, are delivered with the regular batch instead. In metadata-triggered mode only readings inside a metadata window are sent, as before. `getUploadStatistics()` counts these uploads too, so the count rises accordingly
- A sensor returns to the regular upload period when the configuration sends `-1` or `0` for it, or omits the field; a value at or above the regular upload period is accepted but changes nothing. In statistics-only mode the accelerometer, gyroscope and magnetometer keep the regular upload period, because no raw readings are recorded for them there
