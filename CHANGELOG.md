...
- The SDK now records when it comes up and when it is shut down. A device that is running the SDK but not recording produced no rows at all before, so it was indistinguishable in the data from a device where the app was never launched — a silent device is now visibly present. The closing marker is written only by an orderly shutdown, so its absence is itself the signal that the app was killed rather than closed
- Status events — recording start and stop, wake lock, Doze, app foreground and background — are now delivered on every account, not only on accounts that upload around metadata events. In metadata-triggered mode only readings inside a metadata window were ever uploaded, and windows exist only for customer metadata, so a status event that did not happen to land near one was never sent and was eventually deleted. An app that logs no metadata delivered no status events at all. Historical status data is therefore biased rather than merely sparse: what survived is what happened to sit near a delivery event
- The two lifetime markers above are the only events recorded outside a recording. Everything else — recording start and stop, wake lock, Doze, app foreground and background — still requires a session in progress, unchanged. Worth knowing when reading the data: between shutdown and the next start there is nothing to see, by design, and a device that only ever initializes produces one opening row per launch — the closing one appears only when the app shuts the SDK down rather than being killed, which is exactly what makes its absence meaningful

### Bug Fixes

- Fixed the SDK overwriting the `Initialized` status with a permissions request listing no missing permissions. It happened every time the configuration arrived on a fully-granted device, so an app waiting for `Initialized` before starting a recording could wait forever
- Fixed a recording resumed after a restart reporting the wrong origin whenever a start delay is configured: the delay overwrote `resume` with `delayed`, so the one signal that distinguishes "came back after the process died" from "the SDK scheduled a start" was lost on every such host
- Fixed status events being deleted before they could ship, twice over: the 3-day retention horizon erased the shutdown and stop markers persisted for the next launch whenever the app came back after more than three days, and the storage-full cleanup deleted the oldest rows first — which is where a session's opening marker always is. Age-based deletion now spares status rows; they are a few dozen tiny rows a night and are removed once uploaded, so the exemption costs nothing. The storage-full cleanup keeps its own bound anchored on sensor data so a spared status row cannot stall it
- Fixed a shutdown interrupted mid-write dropping the sensor readings it had already taken out of the queue: only the status rows were put back, the rest vanished with the interrupted call
- Fixed the shutdown status event being lost when the app is closed. Shutdown cancels the upload and storage machinery before the event could be written, so it waited in memory for a session that often never came; it is now written to storage during shutdown and sent with the next launch
- Fixed `stopRecording()` writing an end-of-recording event when there was no recording to end — on shutdown, on an authentication failure and when storage filled up. One night with a full disk produced twenty-two such events, which read as a recording stopping over and over
- Fixed data recorded on the way into a traffic limit surviving the wipe that the limit requires: readings still queued when the limit was reached were written to storage after the deletion had already run
- Fixed customer metadata queued at shutdown losing its upload window, which made the readings around that delivery event unuploadable until they were deleted at the retention horizon
