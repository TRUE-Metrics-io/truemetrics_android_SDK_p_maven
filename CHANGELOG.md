...
- `payload_limit_kb` now supports values up to 9,000 KB (previously capped silently at 1,024 KB); larger packets significantly reduce the number of objects written to backend storage
...
- The background service no longer uses the "special use" foreground-service type. If your Google Play foreground-service declaration listed "special use" for the Truemetrics service, update it to "data sync" — the SDK now declares the data-sync type
