...
- Fixed the app crashing when a device's sensor service delivers a malformed reading. The bad reading is now dropped and recording carries on
- Fixed a hardware sensor (accelerometer, gyroscope, magnetometer, barometer or step counter) silently recording nothing for a whole session on devices whose sensor service does not start it correctly at the beginning of the recording. The SDK now keeps trying to start the sensor for up to about a minute and a half; a sensor that merely starts delivering late still records from that moment, as before

### Behaviour Changes

- A hardware sensor that could not be started is recorded as a status event (`sensor_start_failed`) saying which sensor, why (`unavailable`, `permission_missing`, `register_refused` or `no_events`), how many attempts were made and, if the device's sensor service threw, the exception class; a sensor that needed more than one attempt is recorded as `sensor_recovered`. Both are written once per start of that sensor; a sensor the device does not have, or one whose runtime permission the app lacks, is reported once per SDK initialization rather than on every start. A sensor that fails at the start of a recording is now explained in the data instead of looking like a healthy session with that sensor's rows missing
