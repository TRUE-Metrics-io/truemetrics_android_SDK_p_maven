...
- Fixed data uploads that could silently stop for the rest of a session when the SDK was re-initialized while a previous shutdown was still completing in the background (for example right after an authentication error with a corrected API key)
- On Android 15, when the system ends extended background recording (possible if location permission was never granted), the SDK now reports the recording as stopped through the status listener instead of the app continuing to see an active recording that had silently ended
- Fixed a case on Android 14+ where the background recording service could fail to start when location permission was not granted
...
- In metadata-triggered mode, fixed a reading at a packet boundary being included in consecutive packets (duplicated data in uploads), and fixed the tail of a metadata window smaller than the size threshold not being finalized for upload

### Improvements

- In metadata-triggered upload mode, recorded data that has not been uploaded yet is now retained for up to 3 days instead of being deleted once it falls outside the metadata buffer window. A metadata event arriving late can still trigger the upload of the data it covers (for example after the device was offline)
- If device storage fills up before data can be uploaded, the oldest unsent readings are now dropped first so recording continues, instead of new data being lost. The drop never removes the newest readings even when little data is stored (short recordings lose at most the older half), repeated drops are suppressed while they demonstrably don't help (e.g. the disk was filled by another app) and resume automatically once inserts succeed or the SDK is re-initialized, retries are paced (~20s) instead of spinning while the disk stays full, and a full disk hit while saving a metadata event is handled the same way as for sensor data. Retention cleanup hitting a full disk no longer fails an already-saved batch
